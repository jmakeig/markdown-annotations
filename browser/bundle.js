(function () {
  ...
  */
   ...
   * @return {Action}
   */
  function documentLoad({ content, annotations }, href, mime = 'text/markdown') {
    ...
    return file({
      ...
      onchange: evt => {
        ...
        reader.addEventListener('load', () => {
          dispatch(documentLoad(parseAnnotatedMarkdown(reader.result), file$$1.name));
        });
  ...
  function render$6(annotation, isActive = false, isEditing = false, user, markers, dispatch) {
    ...
    if (isActive) {
      ...
      return div(props, div({ className: 'annotation-toolbar' }, render(annotation.user), EditAffordance(annotation, isEditing, user, {
        dispatch,
      ...
      return div(props, { classList: 'collapsed' }, render(annotation.user), commentText);
    }
  }
